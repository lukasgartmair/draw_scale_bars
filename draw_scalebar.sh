...
ly=`convert -format "%[fx:((100*h)/100) + ($height_border*0.7)]"  $image_name info:`
ytext=`convert -format "%[fx:($ly - $height)/1.6 ]"  $image_name info:`
...
convert -stroke black -strokewidth 2 -pointsize 25 -gravity south -draw "line $lsx,$ly $lex,$ly  text 0,$ytext '$scale_label $unit'" $image'_with_border.eps' $image'_with_scalebar.eps'
